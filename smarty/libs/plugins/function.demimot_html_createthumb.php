 ...
 *
 * Author: Andreas B&ouml;sch (AB)
 * 
 * Documentation
 * usage: <!--[ img_createthumb file="images/visuals/rallyewm02/Rallye142.jpg" width="150" link=false html=?class="img float" ]-->
 * 
 * params:
 * file [string] (required): The path to your original big-sized image.
 * width [int] (Standard: 100): The width of your thumbnail. The height (if not set) will be automatically calculated.
 * height [int] (Standard: 100): The height of your thumbnail. The width (if not set) will be automatically calculated.
 * sharpen [bool] (Standard: true): Set to »false« if you don’t want to use the Unsharp-Mask. Thumbnail creation will be faster, but quality is reduced.
 * html [string]: Will be inserted in the image-tag. Useful, to align text around the thumbnail or to insert the alt-paramter or to …
 * link [bool] (Standard: true): If set to »false« the image will not get linked and not have a lens-icon.
 * hint [bool] (Standard: true): If set to »false« the image will get linked but will not have a lens-icon.
 * type [int]: The output file format. Set to 1 for GIF, 2 for JPEG and 3 for PNG. If not set, the source file format will be used.
 * addgreytohint [bool] (Standard: true): Set to »false« to get no lightgrey bottombar.
 * linkurl [string] (Standard: set to »original image« ): Set to your target URL (a href="linkurl").
 * window [bool] (Standard: true): Set to »false« if you don’t want to open original image in a new window.
 * longside [int] (Standard: not set): Set the longest side of the image if width, height and shortside is not set.
 * shortside [int] (Standard: not set): Set the shortest side of the image if width, height and longside is not set.
 * extrapolate [bool] (Standard: true): Set to »false« if your source image is smaller than the calculated thumb and you do not want the image to get extraploated.
 * crop [bool] (Standard: true): If set to »true«, image will be cropped in the center to destination width and height params, while keeping aspect ratio. Otherwise the image will get resized.
 * naked [bool] (Standard: false): If set to »true«, image will be returned without <img ...> tag, only image source
 *
 * Changelog:
...
function smarty_function_demimot_html_createthumb($params, &$smarty) {
	...
	if (empty($params['hint'])) $params['hint'] = true; // if empty = true
	...
	### Info &uuml;ber Source (SRC) holen
	...
	$_SRC['string']		= $temp[3];
	// itevo, /Go
	if (substr($params['file'], 0, 7) == 'http://' or substr($params['file'], 0, 8) == 'https://') {
		$_SRC['filename'] 	= md5($params['file']);
		switch ($_SRC['type']) {
		case 1: 
			$_SRC['filename'] .= '.gif';
			break;
	...
	### Infos &uuml;ber Destination (DST) errechnen
	...
	// Das Gr&ouml;&szlig;enverh&auml;ltnis soll erhalten bleiben egal ob das Bild hoch oder querformatig ist.
	...
	if($params['crop'])
		...
		// es muss an der H&ouml;he beschnitten werden
	...
	// Wenn das Ursprungsbild kleiner als das Ziel-Bild ist, soll nicht hochskaliert werden und die neu berechneten Werte werden wieder &uuml;berschrieben
	...
	### R&uuml;ckgabe-Strings erstellen
	// itevo: changed to get files from outside of the document root
	...
	//if (empty($params['html'])) $_RETURN['img'] = '<img src="' . pnModURL('itevoCORE', 'itevoDOCMAN', 'getCachedImageInline', array('filename' => basename($_DST['file']))) . '" '.$params['html'].' '.$_DST['string'].' alt="" title="" />';
	if ($params['naked']) {
	    $returner = $_DST['file'];
	}
	else {
    	if (empty($params['html'])) {
    	    $_RETURN['img'] = '<img src="' .  $_CONFIG['cache'] . basename($_DST['file']) . '" ' . $params['html'].' ' . $_DST['string'] . ' alt="" title="" />';
    	}
    	else {
    	    $_RETURN['img'] = '<img src="' .  $_CONFIG['cache'] . basename($_DST['file']) . '" '. $params['html'] . ' ' . $_DST['string'] . ' />';
    	}
        // /itevo
    	// itevo, ANS: check the right way
    	// if ($params['link'] == "true")
    	if ($params['link']) {
    	// /itevo	
    		// itevo: changed    
    		...
    		//if (empty($params['linkurl'])) $params['linkurl'] = pnModURL('itevoCORE', 'itevoDOCMAN', 'getImageInline', array('filename' => str_replace(pnConfigGetVar('content') . '/itevoDOCMAN/Images', '', $_SRC['file'])));
    		// /itevo
    		// demimot
			// this is where I'm bringing the new gallery thing instead of a new windows // kkk
			// if boolan flag then bring $_RETURN['img']
			// ELSE leave empty so no thumbs...
			// might work
			// get to set the flag on the first loop... on publist
    		if ($params['window']) {
				if($params['firstpic']==1){
    		    	$returner = '<a href="' . $params['linkurl'] . '" ' . $params['html2'] . ' >' . $_RETURN['img'] . '</a>';
				}else
				{
    		    	$returner = '<a href="' . $params['linkurl'] . '" ' . $params['html2'] . ' ></a>';
				}
				//$returner = '<a href="' . $params['linkurl'] . '" target="_blank">' . $_RETURN['img'] . '</a>'; // original
                //$returner = '<a href="javascript:newWin(\'' . $params['linkurl'] . '\', \'auto\', ' . ($_SRC['width']+35) . ', ' . ($_SRC['height']+35) . ')">' . $_RETURN['img'] . '</a>'; // itevo
    		}
    		else {
	...
	// Wenn das Bild sehr gro&szlig; ist, zuerst linear auf vierfache Zielgr&ouml;&szlig;e herunterskalieren und $_SRC &uuml;berschreiben
	if ($_DST['width']*4 < $_SRC['width'] AND $_DST['height']*4 < $_SRC['height'])
		{
		// Multiplikator der Zielgr&ouml;&szlig;e
	...
	// Soll eine Lupe eingef&uuml;gt werden?
	// itevo, ANS: check teh right way
	// if ($params['hint'] == "true" AND $params['link'] == "true")
	if ($params['hint'] == "true" AND $params['link'])
	// /itevo	
		{
		//Soll der wei&szlig;e Balken wirklich hinzugef&uuml;gt werden?
	...
	// Berechnungszeit hinzuf&uuml;gen
	if ($params['dev'])
		{
		// Zeit anhalten
